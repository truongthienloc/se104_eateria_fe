          ...
          menu.map( (item) => {
            return(
              <NavLink
                key={item.href}
                to={item.href}
                className={'w-[250px] h-[40px] flex items-center justify-center rounded-md text-second border-slate-900 border hover:opacity-80 hover:text-primary transition-all'}
              >
                {item.label}
              </NavLink>
            )

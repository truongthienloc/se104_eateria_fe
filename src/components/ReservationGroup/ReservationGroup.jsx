...
export default function ReservationGroup({title, data}) {
  return (
    <div className='w-96 flex flex-col gap-4 items-center border-primary border p-4 pb-8'>
        ...
        {
            data.map(value => (
                <ReservationItem key={value.name} name={value.name} status={value.status} />
            ))
        }
        </div>
    </div>
...
function ReservationItem({name, status, onClick}) {
    return (
        <button className={clsx("w-16 h-10 flex items-center justify-center bg-[#ECECEC] border-none rounded hover:opacity-80 transition-opacity", {
